...
plots <- lapply(vars.pos, function(var) {
  y0 %>%
    ...
    filter(dep == var) %>%
    add_lines(hovertemplate = paste('<b>Casos Totales</b>: %{x}',
                                    '<br><b>Casos Nuevos</b>: %{y:.2f}<br>'),
              name = ifelse(var == vars.pos[length(vars.pos)],"Media Móvil",var),
              legendgroup = 'group1',
              showlegend = ifelse(var == vars.pos[length(vars.pos)],TRUE,FALSE), 
              line = list(color = "#ffa600", width = 4),
              marker  = list(color = "#ffa600")
...
subplot(plots, nrows = 5, shareX = T, titleX = F,shareY=T, titleY=F) %>%
  layout(margin =0.12,
         title = list(text = "Trayectoria de casos de COVID19",
                      font = list(size = 24,
                                  color = "white")),
         annotations = list(
           list(text = "Total de casos (Logarítmico)",
                x = 0.5,
                y = -0.1,
                yref = "paper",
                xref = "paper",
                ...
                font = list(size = 16,
                            color = "white")),
           list(text = "Casos Nuevos (cada 7 días)",
                x = -0.04,
                y = 0.5,
                yref = "paper",
                xref = "paper",
