...
labels<- sprintf(
  "<strong>%s</strong><br/<strong>Pruebas: </strong>%s",
  c.dep$dep, c.dep$smp.new) %>% lapply(htmltools::HTML)

pal.cases <-  colorNumeric( palette="Blues", domain = log(c.dep$smp.new), na.color="transparent")

leaflet(c.dep) %>% 
  map_tiles()  %>%
  map_poly(c.dep$smp.new)%>%
  addLegend("bottomleft", pal=pal.cases, values = log(c.dep$smp.new), title= 'Pruebas', labFormat = labelFormat(transform = function(x) round(exp(x))))%>%
  map_bounds()  %>%
  addEasyButton(easyButton(
    icon="fa-info-circle", title="Información",
    onClick=JS("function(gd) { 
...
plots <- lapply(vars.pmav.new, function(var) {
  dep %>%
    group_by(dep) %>%
    plot_ly(x = ~dat, y = ~mav.pos.new.hab) %>%
