...
pal.cases <-  colorNumeric( palette="Blues", domain = log(c.dep$smp.imp.new), na.color="transparent")

leaflet(c.dep) %>% 
  map_tiles()  %>%
  map_poly(c.dep$smp.imp.new)%>%
  addLegend("bottomleft", pal=pal.cases, values = log(c.dep$smp.imp.new), title= 'Pruebas', labFormat = labelFormat(transform = function(x) round(exp(x))))%>%
  map_bounds()  %>%
  addEasyButton(easyButton(
    icon="fa-info-circle", title="Información",
    onClick=JS("function(gd) { 
