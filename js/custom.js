$(function(){
  ...
  //Otras enfermedades
  $.ajax({
    url: "formato-otras-enfermedades.csv",
    async: false,
    success: function (csvd) {
      var items = $.csv.toObjects(csvd);
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        div_a = document.createElement('a');
        div_a.className = "link";
        div_a.setAttribute('href', items[i].LINK);
        div_a.setAttribute('target', '_blank');
        div_a.textContent = "Descargar aquí el pdf";
        div_link.appendChild(div_a);
        ...
        //Poner todo dentro de la ruta del DIV que queramos
        var currentDiv = document.getElementById('otras-enfermedades');
        currentDiv.appendChild(div_wrapper);
      }
    }
  });
